...
def affine_forward(x, w, b):
    ...
    Inputs:
    - x: A numpy array containing input data, of shape (N, d_1, ..., d_k)   eg. (2, 4, 5, 6)
    - w: A numpy array of weights, of shape (D, M)                          eg. (120, 3)
    - b: A numpy array of biases, of shape (M,)                             eg. (3,)

    Returns a tuple of:
    - out: output, of shape (N, M)                                          eg. (2, 3)
    - cache: (x, w, b)
    """
    out = None
    ###########################################################################
...
def affine_backward(dout, cache):
    ...
    Inputs:
    - dout: Upstream derivative, of shape (N, M)                    eg. (10, 5)
    - cache: Tuple of:
      - x: Input data, of shape (N, d_1, ... d_k)                   eg. (10, 2, 3)
      - w: Weights, of shape (D, M)                                 eg. (6, 5)
    ...
    ###########################################################################
    trans_x = x.reshape(x.shape[0], -1)
    dtrans_x = dout.dot(w.T)
    dx = dtrans_x.reshape(x.shape)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # variables.                                                          #
        #######################################################################
        ### mean and var computed along the columns
        # mean = np.sum(x, axis=0, keepdims=True)/N
        # std = np.std(x-mean, axis=0, keepdims=True) + eps
        # x_hat = (x-mean) / std
        # y = gamma*x_hat + beta

        ### following decompose the above steps into
        mean = np.sum(x, axis=0, keepdims=True)/N
        x_mean = x - mean
        x_mean_2 = x_mean**2
        var = np.sum(x_mean_2, axis=0, keepdims=True)/N
        sqrtvar = np.sqrt(var + eps)
        invvar = 1/sqrtvar       # shape (D,)
        x_hat = x_mean*invvar
        ...
        cache = gamma, var, eps, x_hat, x_mean, sqrtvar, invvar
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    gamma, var, eps, x_hat, x_mean, sqrtvar, invvar = cache
    N = dout.shape[0]

    dgamma = np.sum(dout*x_hat, axis=0)
    dbeta = np.sum(dout, axis=0)

    ### nice back propogation exercies!
    # y = gamma_x_hat + beta
    # gamma_x_hat = gamma*x_hat
    dx_hat = dout*gamma       # shape (N, D)

    # x_hat = x_mean*invvar
    dinvvar = np.sum(dx_hat*x_mean, axis=0, keepdims=True)
    dx_mean = dx_hat*invvar   # shape (N, D)
    ...
    # var = np.sum(x_mean_2, axis=0, keepdims=True)/N
    dx_mean_2 = dvar*np.ones(dout.shape)/N               # note that the gradients are all one

    # x_mean_2 = x_mean**2
    dx_mean += dx_mean_2*2*x_mean

    # x_mean = x - mean
    dx = dx_mean
    dmean = -np.sum(dx_mean, axis=0, keepdims=True)  # shape (D,)

    # mean = np.sum(x, axis=0, keepdims=True)/N
    dx += dmean*np.ones(dout.shape)/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    N, D = dout.shape
    dx = (1/N)*gamma*(var+eps)**(-0.5) * (N*dout - np.sum(dout, axis=0)- x_hat*np.sum(dout*x_hat, axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
        ...
        for c in range(C):
            for h_prime in range(h_out):
                for w_prime in range(w_out):
                    ...
                    out[n,c,h_prime,w_prime] = np.max(x_pad[c, h1:h2, w1:w2])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for c in range(C):
            for h_prime in range(h_out):
                for w_prime in range(w_out):
                    ...
                    w1 = w_prime*stride
                    w2 = w_prime*stride + pool_width
                    window = x_pad[c, h1:h2, w1:w2] * (1/np.max(x_pad[c, h1:h2, w1:w2])) * \
                            (x_pad[c, h1:h2, w1:w2] >= np.max(x_pad[c, h1:h2, w1:w2]))
                    dx[n,c,h1:h2,w1:w2] = dout[n,c,h_prime,w_prime]*window
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_vanilla = reshape_to_bn(x, *x.shape)
    out, bn_cache = batchnorm_forward(x_vanilla, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_vanilla, dgamma, dbeta = batchnorm_backward_alt(dout, bn_cache)
    dx = reshape_from_bn(dx_vanilla, *x_shape)
